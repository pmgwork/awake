...
- **バッテリー保護** — 指定した残量（オフ／5〜25%、初期値20%）で自動停止し、ファンも自動制御へ戻します。
...
- モード
  ...
  - *無期限* — 手動で停止するまで
- 設定
  - **一般** — ログイン時に起動、通知、メニューバーの残り時間表示、バッテリー停止、完了後の猶予時間、画面とスクリーンセーバー、ダウンロードフォルダ、グローバルショートカット、アップデート
...
タグ `v0.2.0` で GitHub Release を作成し、**stable**（draft でも pre-release でもない状態）にして ZIP を添付します。アプリの更新確認は `releases/latest` を参照するため、draft と pre-release は対象外です。

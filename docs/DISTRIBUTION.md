...
- タグ形式: `vX.Y.Z` (例 `v1.0.1`)。`AppUpdateService` が先頭 `v` を剥がして
  `CFBundleShortVersionString` と比較する。

## リリース手順

1. 版数を決める: `MARKETING_VERSION` / `CURRENT_PROJECT_VERSION`
   (Xcode > Target awake > General、現在 `1.0`)
2. パッケージ作成:

   ```sh
   scripts/package.sh 1.0.1
   # => dist/Awake-1.0.1.zip (+ .dmg), dist/Awake-1.0.1.sha256
   ```

3. GitHubでタグ + Release作成:
   - タグ `v1.0.1` をpush
   - Release名 `Awake v1.0.1`、Notesに変更点とSHA256を記載
   - `dist/Awake-1.0.1.zip` (必須) を添付。DMGは任意。
